"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.projectService = void 0;
const projects_model_1 = require("./projects.model");
// create project
const createProjectIntoDB = (payload) => __awaiter(void 0, void 0, void 0, function* () {
    const result = yield projects_model_1.Project.create(payload);
    return result;
});
// Get Single Project
const getSingleProjectFromDb = (id) => __awaiter(void 0, void 0, void 0, function* () {
    const result = yield projects_model_1.Project.findById(id);
    return result;
});
// delete Single Project
const removeSingleProjectFromDb = (id) => __awaiter(void 0, void 0, void 0, function* () {
    yield projects_model_1.Project.findById(id);
    return null;
});
// Get All Project
const AllProjectFromDb = () => __awaiter(void 0, void 0, void 0, function* () {
    const result = yield projects_model_1.Project.find();
    return result;
});
exports.projectService = {
    createProjectIntoDB,
    getSingleProjectFromDb,
    AllProjectFromDb,
    removeSingleProjectFromDb,
};
